...
class MonitoredURL(models.Model):
    ...
    webhook_url = models.URLField(
        null=True, blank=True, help_text="Optional webhook for alerts")
...
class UptimeHistory(models.Model):
    monitored_url = models.ForeignKey(
        MonitoredURL, on_delete=models.CASCADE, related_name="history")
    status = models.CharField(
        max_length=10, choices=MonitoredURL.STATUS_CHOICES)
